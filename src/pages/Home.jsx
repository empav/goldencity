...
function Home() {
  ...
  return (
    <div className="space-y-16 dark:bg-gray-950 dark:text-gray-100">
      {/* Hero Section */}
      ...
      <section className="container">
        <div className="text-center mb-12">
          ...
          <p className="text-secondary-600 dark:text-gray-300">Your journey to crypto-powered real estate investment</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {investmentSteps.map((step, index) => (
            ...
            >
              <div className="bg-white p-6 rounded-lg shadow-md text-center dark:bg-gray-900 dark:shadow-gray-950/20">
                <div className="bg-primary-50 w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 dark:bg-primary-900/40">
                ...
                <p className="text-secondary-600 dark:text-gray-300">{step.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
      ...
      <section className="container">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {featuredProperties.map((property, index) => (
            <motion.div
              key={property.id}
              className="card group dark:bg-gray-900 dark:shadow-gray-950/20"
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.2 }}
            >
              <div className="relative h-48">
                ...
                />
                <div className="absolute top-4 right-4 bg-white px-3 py-1 rounded-full text-primary-600 font-semibold dark:bg-gray-900/90 dark:text-primary-400">
                  {property.status}
                </div>
              </div>
              <div className="p-6">
                <h3 className="text-xl font-semibold mb-2">{property.title}</h3>
                <p className="text-secondary-600 mb-4 dark:text-gray-300">{property.location}</p>

                <div className="flex justify-between items-center mb-4">
                  <div>
                    <p className="text-sm text-secondary-500 dark:text-gray-400">Price</p>
                  ...
                  <div className="text-right">
                    <p className="text-sm text-secondary-500 dark:text-gray-400">ROI</p>
                ...
                <div className="space-y-2 mb-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-secondary-600 dark:text-gray-300">Total Investors</span>
                    <span className="font-medium">{property.metrics.totalInvestors}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-secondary-600 dark:text-gray-300">Funded</span>
                    <span className="font-medium">{property.metrics.funded}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-secondary-600 dark:text-gray-300">Min Investment</span>
                    <span className="font-medium">{property.metrics.minInvestment}</span>
                  </div>
                </div>

                <Link to={`/properties/${property.id}`} className="btn w-full flex items-center justify-center">
                  Invest Now
      ...
      {/* Why Choose Us */}
      <section className="bg-secondary-50 pt-16 dark:bg-gray-900/50">
        <div className="container">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold mb-4">Why Choose GoldenCity</h2>
            <p className="text-secondary-600 dark:text-gray-300">Experience the future of real estate investment</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {advantages.map((advantage, index) => (
              <motion.div
                key={index}
                className="bg-white p-6 rounded-lg shadow-md text-center dark:bg-gray-900 dark:shadow-gray-950/20"
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ delay: index * 0.2 }}
              >
                ...
                <p className="text-secondary-600 dark:text-gray-300">{advantage.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      {/* Blog */}
      <div className="container bg-white py-24 dark:bg-gray-950">
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {blogPosts.map((post, index) => (
            <motion.article
              ...
              className="bg-white rounded-lg shadow-md overflow-hidden dark:bg-gray-900 dark:shadow-gray-950/20"
            >
              <Link to={`/blog/${post.slug}`}>
                <div className="relative h-48">
                  <img src={post.image} alt={post.title} className="w-full h-full object-cover" />
                  <div className="absolute top-4 right-4 bg-white px-3 py-1 rounded-full text-sm font-medium text-primary-600 dark:bg-gray-900/90 dark:text-primary-400">
                    {categories.find((c) => c.id === post.category)?.name}
                  </div>
                </div>
                <div className="p-6">
                  <h2 className="text-xl font-semibold mb-3 transition-colors hover:text-primary-600 dark:hover:text-primary-400">
                    {post.title}
                  </h2>
                  <p className="text-secondary-600 mb-4 dark:text-gray-300">{post.excerpt}</p>
          ...
          <div className="space-y-8">
            {faqSections.map((section, sectionIndex) => (
              <div
                ...
                className="bg-white rounded-lg shadow-md overflow-hidden dark:bg-gray-900 dark:shadow-gray-950/20"
              >
                <div className="divide-y divide-secondary-100 dark:divide-gray-800">
                    ...
                    <div key={questionIndex} className="p-6">
                      ...
                      <AnimatePresence>
                        {openSections[`${section.title}-${questionIndex}`] && (
                          ...
                          >
                            <p className="mt-4 text-secondary-600 dark:text-gray-300">{item.answer}</p>
                          </motion.div>
                        )}
                      </AnimatePresence>
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="lg:grid lg:grid-cols-2 lg:gap-8 items-center">
            <div>
              ...
              <dl className="mt-8 space-y-6">
                <div className="flex">
                  ...
                  <div className="ml-4">
                    <dt className="text-lg leading-6 font-medium text-white">10,000+ Members</dt>
                ...
                <div className="flex">
                  ...
                  <div className="ml-4">
                    <dt className="text-lg leading-6 font-medium text-white">Weekly Events</dt>
                    <dd className="mt-2 text-base text-primary-100">
            ...
            <div className="mt-12 lg:mt-0 flex justify-center">
              <div className="bg-white rounded-lg shadow-xl p-8 max-w-sm w-full dark:bg-gray-900 dark:shadow-gray-950/20">
                <h3 className="text-2xl font-bold text-gray-900 text-center mb-6 dark:text-gray-100">Join Discord</h3>
                <p className="text-gray-500 text-center mb-8 dark:text-gray-300">
